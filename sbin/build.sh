...
configuringVersionStringParameter()
{
  ...
  if [ "${BUILD_CONFIG[OPENJDK_CORE_VERSION]}" == "${JDK8_CORE_VERSION}" ]; then
    ...
    addConfigureArg "--with-extra-cflags" "-Ofast -march=x86-64 -mtune=broadwell -funroll-loops -fomit-frame-pointer"
    addConfigureArg "--with-extra-cxxflags" "-Ofast -march=x86-64 -mtune=broadwell -funroll-loops -fomit-frame-pointer"
